...
public class CSVGenerator {
	 ...
	 *         </ul>
	 */
	public boolean generateCSVFile(String outputPath) {
		try {
//			writer = new PrintWriter(new BufferedWriter(new FileWriter(project.getGenerateCSVDir()+"//"+ project.getGenerateCSVFileName())));
			writer = new PrintWriter(new BufferedWriter(new FileWriter(outputPath+"\\"+ project.getGenerateCSVFileName())));
			...
			writer.print("\"CloneSet ID\",");
			writer.print("\"CloneSet CATEGORY\",");
			writer.print("\"Clone ID\",");
			writer.print("\"Clone CATEGORY\",");
			writer.print("\"VERSION\",");
			if(project.isGitDirect()) {
				writer.print("\"COMMITID\",");
			}
			writer.print("\"FILE\",");
			...
			for (CloneSet cloneSet : project.getCloneSetList()) {
				for (Clone clone : cloneSet.getNewCloneList()) {
					...
					writer.print("\"NEW_ "+ project.getAnalysisdate() + "\",");
					if(project.isGitDirect()) {
						writer.print("\"" + project.getCommitIDList().get(project.getAnalysistime()) + "\",");
					}
				...
				for (Clone clone : cloneSet.getOldCloneList()) {
					...
					writer.print("\"" + clone.getOutputId() + "\",");
					writer.print("\"" + clone.getCategoryString() + "\",");
					writer.print("\"OLD_ "+ project.getOldAnalysisdate() + "\",");
					if(project.isGitDirect()) {
						writer.print("\"" + project.getCommitIDList().get(project.getAnalysistime()-1) + "\",");
					}
					writer.print(",,,,,");
					writer.print("\"" + clone.getFile().getName() + "\",");
					writer.printf("%d,", clone.getStartLine());
