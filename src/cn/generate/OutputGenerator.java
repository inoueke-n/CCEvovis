...
public class OutputGenerator {
	...
	public OutputGenerator(Project project) {
		...
		String month;
		String day;
		if(project.getCurrentdate() >1000) {
			String curday = Integer.toString(project.getCurrentdate());
			year = curday.substring(0,4);
			month = curday.substring(4,6);
			day = curday.substring(6,8);
		}else {
	...
	public boolean generateCSVFile() {
		project.setGenerateCSVFileName( project.getAnalysisdate() +  ".csv" );

		if(project.isGenerateHtml()) {
			File fileforHTML = new File(project.getGenerateHTMLDir() + "\\users\\" + project.getUserId() + "\\projects\\" + project.getName() + "\\" +  project.getAnalysisName() + "\\" + project.getAnalysisdate() + "\\");
			if (!fileforHTML.exists()) {
				fileforHTML.mkdirs();
			}
			CSVGenerator generater_csvforHTML = new CSVGenerator(project);
			if (!generater_csvforHTML.generateCSVFile(project.getGenerateHTMLDir() + "\\users\\" + project.getUserId() + "\\projects\\" + project.getName() + "\\" +  project.getAnalysisName() + "\\" + project.getAnalysisdate() + "\\")) {
				Logger.writeln("Can't generate CSV file for HTML", Logger.ERROR);
				return false;
			}
			generater_csvforHTML = null;
		}

		File file = new File(project.getGenerateCSVDir() + "\\" + project.getName() + "\\" +  project.getAnalysisName() + "\\" );
		if (!file.exists()) {
			file.mkdirs();
		}
		...
		if (!generater_csv.generateCSVFile(project.getGenerateCSVDir() + "\\" + project.getName() + "\\" +  project.getAnalysisName() + "\\")) {
