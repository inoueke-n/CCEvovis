...
public class AnalyzeManager {
	 ...
	 *         </ul>
	 */
	public static boolean getCloneInf(Project project, int i) {
		...
		if (project.getTool().equals(Def.CCFX_TOOLNAME)) {
		...
		} else if (project.getTool().equals(Def.ICV_TOOLNAME)) {
			ICVoltiController controller = new ICVoltiController(project);
		    ...
		    controller = null;
		} else if (project.getTool().equals(Def.NO_TOOLNAME)) {
			NoToolController controller = new NoToolController(project);

			// NoToolの実行
			if (!controller.execute()) {
				Logger.writeln("Can't execute NoTool.", Logger.ERROR);
				return false;
			}
...
/*		if (CloneSetCategorizer.searchSimCloneSet(project)) {
		...
		if (project.getTool().equals(Def.CCV_TOOLNAME) || project.getTool().equals(Def.CCFX_TOOLNAME)
				|| project.getTool().equals(Def.SCC_TOOLNAME) || project.getTool().equals(Def.NO_TOOLNAME)) {
